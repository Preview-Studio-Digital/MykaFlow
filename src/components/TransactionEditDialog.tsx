  ...
  useEffect(() => {
    async function fetchCats() {
      try {
        ...
        const official: any[] = [];
        const { data: fromTxs } = await supabase
          .from("transactions")
          .select("category, description, type");

        const merged: any[] = (official || []).map((c) => ({ ...c, isTemporary: false }));
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-background/95 backdrop-blur-md p-4 animate-in fade-in duration-300">
      <div className="glass w-full max-w-xl rounded-3xl p-8 shadow-2xl relative animate-in zoom-in duration-200 border border-white/20 max-h-[95vh] overflow-y-auto">
        <button
          onClick={onClose}
          className="absolute right-6 top-6 text-muted-foreground hover:text-white transition-colors z-10 p-2"
        >
          <X className="h-6 w-6" />
        </button>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className={`flex items-center justify-center gap-2 rounded-xl py-4 text-[10px] font-black uppercase tracking-widest transition-all ${
                type === "expense"
                  ? "bg-destructive/20 text-destructive border border-destructive/60 shadow-[0_0_15px_rgba(239,68,68,0.3)]"
                  : "border border-border/50 text-muted-foreground opacity-50 hover:opacity-100"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 rounded-xl py-4 text-[10px] font-black uppercase tracking-widest transition-all ${
                type === "income"
                  ? "bg-accent/20 text-accent border border-accent/60 shadow-[0_0_15px_rgba(34,211,238,0.3)]"
                  : "border border-border/50 text-muted-foreground opacity-50 hover:opacity-100"
              }`}
            >
            ...
            <label className="block">
              ...
              >
                ...
                {currentParents.map((c) => (
                  <option key={c.id} value={c.id} className="bg-popover uppercase">
                    {c.name}
                  </option>
                ))}
              </select>
            ...
            <label className="block">
              ...
              {currentSubs.length > 0 ? (
                ...
                >
                  <option value="">SELECIONE...</option>
                  {currentSubs.map((s) => (
                    <option key={s} value={s} className="bg-popover uppercase">
                      {s}
                    </option>
                  ))}
                </select>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <label className="block">
              ...
              >
                ...
                <option value="fixed" className="bg-popover uppercase">
                  FIXA
                </option>
              </select>
            </label>
